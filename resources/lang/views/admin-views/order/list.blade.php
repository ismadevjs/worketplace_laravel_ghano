        ...
        <div class="card">
            ...
            <div class="card-header">
                <div class="row justify-content-between align-items-center flex-grow-1">
                    ...
                    @if (Request::url() == 'https://worketplace.com/admin/orders/list/confirmed')
                        
                    <div class="row col-6">
                        @if(count($orders)) 
                        
                        <form action="{{route('admin.orders.enexpedition')}}" method="POST">
                            @csrf
                            <input type="hidden" value="test" name="test">
                            <button class="btn btn-primary"> {{trans('messages.Processing')}} </Button>

                        </form>

                        @endif
                    
                        <form action="{{route('admin.orders.download-excel-confirmed')}}" method="get">
                            <button class="btn btn-success ml-2"> En-Excel </Button>
                        </form>
                    
                    </div>
                    @endif

                   {{-- http://localhost:8888/6-valley-worketplace/admin/orders/list/pending --}}
                                ...
                                <td class="text-capitalize">
                                    @if ($order['order_status'] == 'pending')
                                        <span class="badge badge-soft-warning ml-2 ml-sm-3">
                                            <span
                                                class="legend-indicator bg-warning"></span>{{ trans('messages.'.str_replace('_', ' ', $order['order_status'])) }}
                                        </span>

                                    @elseif($order['order_status']=='processing')
                                        <span class="badge badge-soft-primary ml-2 ml-sm-3">
                                            <span
                                                class="legend-indicator bg-primary"></span>{{ trans('messages.Processing') }}
                                        </span>
                                     @elseif($order['order_status']=='confirmed')
                                        <span class="badge badge-soft-secoundary ml-2 ml-sm-3">
                                            <span
                                                class="legend-indicator bg-secoundary"></span>{{ trans('messages.'.str_replace('_', ' ', $order['order_status'])) }}
                                        </span>
                                    @elseif($order['order_status']=='canceled')
                                        <span class="badge badge-soft-danger ml-2 ml-sm-3">
                                            <span
                                                class="legend-indicator bg-danger"></span>{{ trans('messages.'.str_replace('_', ' ', $order['order_status'])) }}
                                        </span>
                                    @elseif($order['order_status']=='delivered')
                                        <span class="badge badge-soft-success ml-2 ml-sm-3">
                                            <span
                                                class="legend-indicator bg-success"></span>{{ trans('messages.'.str_replace('_', ' ', $order['order_status'])) }}
                                        </span>
                                    @else
                                        <span class="badge badge-soft-danger ml-2 ml-sm-3">
                                            <span
